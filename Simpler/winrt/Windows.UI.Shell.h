...
namespace winrt::impl
{
    ...
    template <typename U> struct bindable<Windows::UI::Shell::TaskbarManager, U>
    {
        static xaml_binding bind(Windows::UI::Shell::TaskbarManager const& object, hstring const& name)
        {
            if (name == L"IsSupported") return
            {
                [object] { return object.IsSupported(); }
            };
            if (name == L"IsPinningAllowed") return
            {
                [object] { return object.IsPinningAllowed(); }
            };
            return {};
        }
    };
